...
The greedy-tuned XGBoost model (Table 7) performs best overall, which has the best accuracy score among all models. The table show very high recall and specificity for damage_grade = 1 and 2, with a relatively lower but fairly reasonable recall for damage_grade = 3. Also, ROC curves from the final plot for XGBoost also show strong separation between damage_grade classes. Therefore, we used it to generate damage_grade predictions for all building ID in nepal_test.
...
If we would have the chance to approach this case study again, we will ask for more information of geological id variables because they seem to be the most relevant with damage grade of buildings, yet due to overwhelming amount of levels, it is hard to utilize them in prediction tasks, especially in linear models. Though ensemble methods deal with high dimensional very well, the amount of trees and their depths required to make meaningful prediction is demanding. Thus, if we have more infromation, we could have redesign the variables so that they are easier to work with.
