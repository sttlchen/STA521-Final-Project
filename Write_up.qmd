...
The greedy-tuned XGBoost model achieved an overall accuracy of 82.8% and a balanced accuracy of 85.8% on the held-out test data. The ROC curves demonstrate excellent class separability, with AUC values of 0.993 for damage grade 1, 0.908 for grade 2, and 0.912 for grade 3, yielding a macro-AUC of 0.937.

Misclassifications occur primarily between adjacent damage levels, which is expected for ordinal severity data. Importantly, the model rarely confuses high-severity (grade 3) buildings with low-severity (grade 1), indicating strong reliability for practical damage assessment.

Overall, the model shows strong predictive performance, excellent discriminative ability, and is well-suited for classifying post-earthquake building damage.
