...
For XGBoost, we conducted a small grid search over max_depth and learning rate on SMOTE-balanced data, selecting the settings that maximized balanced accuracy. The final model was trained with subsampling and column subsampling to control overfitting. As a robustness check, we also experimented with an Isolation Forest on numeric predictors to flag extreme outliers. Removing the top 2% most anomalous observations, we refit the full set of models to assess how sensitive results were to extreme cases.
...
The multinomial GLM (Table 2) performs poorly, with 56% accuracy, Kappa = 0.28, and modest balanced accuracy. It overpredicts damage_grade = 2 and fails to separate the other classes because its linear structure cannot capture complex interactions in the data.

The baseline Random Forest (Table 3) performs much better, reaching ≈79% accuracy, Kappa ≈ 0.67, and balanced accuracy ≈ 83%. It substantially improves sensitivity for classes 1 and 3 compared with the GLM, though some confusion between grades 2 and 3 remains.
...
CatBoost (Table 6) outperforms the Random Forest models, achieving ≈81.7% accuracy, Kappa ≈ 0.72, and balanced accuracy ≈ 84.9%. Its strength comes from handling many categorical variables effectively, resulting in more consistent performance across all three damage grades. The ROC curves show high AUC values for each class, indicating strong classification ability.

The greedy-tuned XGBoost model (Table 7) performs best overall, with the highest accuracy and strong sensitivity and specificity for damage grades 1 and 2, and reasonable performance for grade 3. Its ROC curves show clear separation among classes. Because of this superior performance, we used XGBoost to generate the final damage-grade predictions for the nepal_test dataset.
