...
```{r}
# Cell 1 — First 10 categorical variables 
...
```{r}
# Cell 2 — Second 10 categorical variables 
nepal |>
...
```{r}
# Cell 3 — Last 10 categorical variables
nepal |>
...
sm_out <- SMOTE(
  X = as.data.frame(X_num), target = y, K = 5,
  dup_size = dup_size
)
...
ord_model <- polr(
  damage_grade ~ factor(geo_level_1_id) + factor(geo_level_2_id) + factor(geo_level_3_id) +
    count_floors_pre_eq + age + area_percentage + height_percentage +
    land_surface_conditionn + land_surface_conditiono + land_surface_conditiont 
    + foundation_typei + foundation_typer + foundation_typeu + foundation_typew 
    + roof_typeq + roof_typex + ground_floor_typev + ground_floor_typex + ground_floor_typez
    + ground_floor_typeOther + other_floor_typeq + other_floor_types + other_floor_typex
    + positiono + positions + positiont + count_families + has_superstructure_mud_mortar_stone1,
  data = glm_dat,
  Hess = TRUE
)
...
library(caret)
# Predicted values
preds <- predict(ord_model, newdata = glm_dat)

# Confusion matrix
confusionMatrix(preds, glm_dat$damage_grade)
